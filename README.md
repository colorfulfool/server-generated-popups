*Ruby on Rails gives our applications clear-cut structure: one screen transition — one HTTP request — one ERB view. I love it. Click "New" button — that's a controller action. Click "Edit" button — that's a controller action. But what if your edit screen is a pop-up dialog? **Uuuuugh.** Now you have to embed your view for edit action into the view for index action AND stick a JS click handler somewhere to pop that up. This disrupts our clear-cut structure. Damn. What if your controller could respond with a popup instead of a page visit? Well guess what, now it can!*
